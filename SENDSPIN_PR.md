...
Unlike Bluetooth, AirPlay, and Spotify renderers which need runtime ALSA device switching (e.g., local output vs btstream), SendSpin uses a single static ALSA device (`sendspin`) defined in `/etc/alsa/conf.d/sendspin.conf`. No `configureAlsaForSendspin()` function is needed — the ALSA config is regenerated by `generateSendspinService()` on every config save and the device never changes at runtime.
