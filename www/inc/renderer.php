...
function configureAlsaForSendspin($enable) {
	// SendSpin uses a dedicated ALSA device defined in /etc/alsa/conf.d/sendspin.conf
	// This config is regenerated by generateSendspinService() on every config save
	// with the correct card number from the database. No runtime ALSA switching needed.
	// The function exists to match the pattern of other renderers (e.g., Bluetooth)
	// but SendSpin's ALSA config is static and managed through the service file.
	workerLog('configureAlsaForSendspin(): ' . ($enable ? 'start' : 'stop') . ' — ALSA managed by sendspin.conf');
	return true;
}
